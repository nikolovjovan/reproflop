...
{
    printf("Sequential (gold) implementation!\n");
...
void gridding_omp_mem(unsigned int n, parameters params, ReconstructionSample *sample, float *LUT, unsigned int sizeLUT,
                      cmplx *gridData, float *sampleDensity, double *time)
{
    printf("Parallel implementation using thread-local output buffers!\n");

    int numThreads = omp_get_max_threads();

    if (numThreads == 1) {
        gridding_seq(n, params, sample, LUT, sizeLUT, gridData, sampleDensity, time);
        return;
    }
    ...
    cmplx **tGridData = (cmplx **) calloc(numThreads, sizeof(cmplx *));
    float **tSampleDensity = (float **) calloc(numThreads, sizeof(float *));
...
void gridding_omp_locks(unsigned int n, parameters params, ReconstructionSample *sample, float *LUT, unsigned int sizeLUT,
                        cmplx *gridData, float *sampleDensity, double *time)
{
    printf("Parallel implementation using locks!\n");
                  ...
                  cmplx *gridData, float *sampleDensity, double *time)
{
    ...
    if (requiredMemSize > totalMemSize * MAX_MEM_USAGE_PERCENT) {
        gridding_omp_locks(n, params, sample, LUT, sizeLUT, gridData, sampleDensity, time);
    } else {
        gridding_omp_mem(n, params, sample, LUT, sizeLUT, gridData, sampleDensity, time);
    }
}