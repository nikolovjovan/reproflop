...
/*
 * Reads COO data from Matrix Market file.
 *
 * Inputs:
 ...
 *   nz - number of non-zero entries in the matrix
 *   entries - entries in COO format
 */
...
)
{
    ...
    bool mirrored = mm_is_symmetric (matcode);
    bool binary = mm_is_pattern (matcode);
    ...
    int* data_ptr_len)
{
    ...
    read_coo_from_mtx_file(mtx_filename, dim, &rows, &cols, &nz, &entries);

    int result = coo_to_jds(rows, cols, nz, entries, pad_rows, warp_size, pack_size, debug_level,
        data, data_row_ptr, nz_count, data_col_index, data_row_map, data_cols, len, nz_count_len, data_ptr_len);
    ...
    int* data_ptr_len)
{
    ...
    *data_row_map = (int*) calloc(rows, sizeof(int));
    ...
    //printf("First row %d\n", irow);
    for (int i = 0; i < nz; i++) { // loop through each sorted entry
    ...
    *len = total_size;
    ...
    while (1) {
        ...
        for (int i = 0; i < rows; i++) { // data index, including padding
...
/*
 * COO to CSR matrix conversion.
 *
 * Inputs:
 *   mtx_filename - the file in COO format
 *   debug_level - 0 for no output, 1 for simple CSR data, 2 for visual grid
 * Outputs:
 *   dim - dimensions of the input matrix
 *   data - the raw data in CSR format
 *   data_row_ptr - pointer offset into the `data` output, referenced
 *      by the current row loop index
 *   data_col_index - corresponds to the col that the same
 *      array index in `data` is at
    ...
    int** data_col_index)
{
    ...
    int result = coo_to_csr(rows, cols, nz, entries, debug_level, data, data_row_ptr, data_col_index);
    ...
    int** data_col_index)
{
    ...
    //
    if (debug_level >= 1) {
        printf("Allocating data space: %d entries\n", nz);
    }

    *data = (float*) calloc(nz, sizeof(float));
    *data_col_index = (int*) calloc(nz, sizeof(int));
    *data_row_ptr = (int*) calloc(rows + 1, sizeof(int));

    // Compute row pointers using cumulative sum.
    // Row pointers are needed now to index elements while inserting into `data` array.
    //
    for (int i = 0; i < nz; ++i) {
        (*data_row_ptr)[entries[i].row]++;
    }

    for (int i = 0, cumsum = 0; i <= rows; ++i) {
        int nz_in_row = (*data_row_ptr)[i];
        (*data_row_ptr)[i] = cumsum;
        cumsum += nz_in_row;
    }

    for (int i = 0; i < nz; ++i) {
        (*data)[(*data_row_ptr)[entries[i].row]] = entries[i].val;
        (*data_col_index)[(*data_row_ptr)[entries[i].row]] = entries[i].col;
        (*data_row_ptr)[entries[i].row]++; // increment row iterator
    }

    // Reset row indexes which have been used as iterators above.
    //
    for (int i = 0, prev = 0; i < rows; ++i) {
        int next = (*data_row_ptr)[i];
        (*data_row_ptr)[i] = prev;
        prev = next;
    }
