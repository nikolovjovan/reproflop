...
extern void gridding_seq(unsigned int n, parameters params, ReconstructionSample *sample, float *LUT, unsigned int sizeLUT,
...
int main(int argc, char *argv[])
{
    ...
    CALLOC_AND_CHECK (/* name */ gridData_gpu_rep,              /* type */ cmplx,   /* size */ gridNumElems,    /* errCode */ 2);
    CALLOC_AND_CHECK (/* name */ sampleDensity_seq,             /* type */ float,   /* size */ gridNumElems,    /* errCode */ 3);
    CALLOC_AND_CHECK (/* name */ sampleDensity_gpu_rep,         /* type */ float,   /* size */ gridNumElems,    /* errCode */ 4);
    ...
    double tstart, time_loop_seq, time_loop_gpu_rep;
    double time_seq = 0, time_gpu_rep = 0;
    ...
    if (params.useLUT) {
        ...
        calculate_LUT(beta, params.kernelWidth, &LUT_seq, &sizeLUT_seq);
        time_seq += omp_get_wtime() - tstart;
        
        /* add LUT calculation time to reproducible time, no need to calculate LUT twice */
        time_gpu_rep += time_seq;
    }

    printf("\nLUT generation execution time: %.3f\n", time_seq);
    ...
    tstart = omp_get_wtime();
    gridding_seq(n, params, samples, LUT_seq, sizeLUT_seq, gridData_gpu_rep, sampleDensity_gpu_rep, true, &time_loop_gpu_rep);
    time_gpu_rep += omp_get_wtime() - tstart;

    printf("\nReproducible gpu implementation loop execution time: %.3f\n", time_loop_gpu_rep);
    printf("\nReproducible gpu implementation execution time: %.3f\n", time_gpu_rep);

    printf("\nTime loop execution gpu reproducible / sequential non-reproducible: %.3f\n", time_loop_gpu_rep / time_loop_seq);
    printf("\nTime gpu reproducible / sequential non-reproducible: %.3f\n", time_gpu_rep / time_seq);
    ...
    free(samples);
    free(gridData_seq);
    free(gridData_gpu_rep);
    free(sampleDensity_seq);
    free(sampleDensity_gpu_rep);
