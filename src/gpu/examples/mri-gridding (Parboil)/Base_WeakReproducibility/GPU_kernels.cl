                             ...
                             unsigned int binsize, unsigned int gridNumElems)
{
    ...
    if (sampleIdx < n)
    {
        ...
        binIdx = (unsigned int)(pt.kZ) * ((int)(SIZE_XY_VAL)) +
                 (unsigned int)(pt.kY) * ((int)(GRIDSIZE_VAL1)) +
                 (unsigned int)(pt.kX);
...
__kernel void reorder_kernel(int n, __global unsigned int *idxValue_g,
...
float kernel_value(float v)
{
    ...
    float num =
        (z * (z * (z * (z * (z * (z * (z * (z * (z * (z * (z * (z * (z * (z * 0.210580722890567e-22f +
...
__kernel void gridding_GPU(__global ReconstructionSample *sample_g,
                           ...
                           __global float *sampleDensity_g, float beta)
{
    ...
    const int z0 = get_local_size(2) *
                   (get_group_id(1) / (GRIDSIZE_VAL2 / get_local_size(1)));
    const int y0 = get_local_size(1) *
                   (get_group_id(1) % (GRIDSIZE_VAL2 / get_local_size(1)));
    ...
    for (int z = zL; z <= zH; z++)
    {
        for (int y = yL; y <= yH; y++)
        {
            __global const unsigned int *addr =
                binStartAddr_g + z * SIZE_XY_VAL + y * GRIDSIZE_VAL1;
            ...
            {
                int tileSize = ((delta - (x << LOG_TILE)) > TILE)
                                   ? TILE
                                   : (delta - (x << LOG_TILE));
                ...
                for (int j = 0; j < tileSize; j++)
                {
                    ...
                    if ((real != 0.0f || imag != 0.0f) && sdc != 0.0f)
                    {
                        ...
                        {
                            const float w =
                                kernel_value(beta * sqrt(1.0f - (v * ONE_OVER_CUTOFF2_VAL))) *
                                sdc;
                            pt.x += w * real;
                            pt.y += w * imag;
                            density += 1.0f;
