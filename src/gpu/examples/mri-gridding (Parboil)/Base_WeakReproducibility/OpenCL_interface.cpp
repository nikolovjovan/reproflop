...
/***********************************************************************
 * OpenCL_interface is the main function for GPU execution. This
    ...
    size_t *workItemSizes) // maximum size of work-items for each dimension
{
    ...
    if (zeroData == NULL)
    {
        fprintf(stderr, "Could not allocate dummy memset memory\n");
        exit(1);
    }
    ...
    sortedSample_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, n * sizeof(ReconstructionSample), zeroData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
    binStartAddr_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, (gridNumElems + 1) * sizeof(unsigned int), zeroData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
    sample_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, n * sizeof(ReconstructionSample), sample, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
    idxKey_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, (((n + 3) / 4) * 4) * sizeof(unsigned int), maxIntData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum); // Pad to nearest multiple of 4 to
    idxValue_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, (((n + 3) / 4) * 4) * sizeof(unsigned int), zeroData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum); // satisfy a property of the sorting kernel.
    ...
    const char *source_path = "./GPU_kernels.cl";
    char *source;
    ...
    */
    ...
    cl_mem dkeys_o = clCreateBuffer(clContext, CL_MEM_READ_WRITE, n * sizeof(unsigned int), NULL, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
    cl_mem dvalues_o = clCreateBuffer(clContext, CL_MEM_READ_WRITE, n * sizeof(unsigned int), NULL, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
     ...
     * bins.
     */
    gridData_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, gridNumElems * sizeof(cmplx), zeroData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
    sampleDensity_d = clCreateBuffer(clContext, CL_MEM_COPY_HOST_PTR, gridNumElems * sizeof(float), zeroData, &ciErrNum);
    OCL_ERRCK_VAR(ciErrNum);
